...
- Добавлен baseline PMM (`pmm`): stack-based allocator страниц для внутренних нужд ядра (предвыделенный пул до подключения memory map).
...
нерекомендованный видеодевайс. В `1.15.0` расширен перебор VBE-режимов, в `1.15.1`
исправлена загрузка больших `kernel.bin` (чтение с диска без перехода через 64 KiB-границу
одним BIOS-вызовом), а в `1.15.2` исправлен IRQ-регресс в IDT-stub (полное сохранение
контекста + отложенный `sti` после регистрации обработчиков в `kmain`). Для WoOS сначала
проверьте базовый VGA-совместимый путь:
```bash
qemu-system-x86_64 \
  -drive format=raw,file=os.img \
  -m 1024 \
