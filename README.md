...
- Добавлен VFS skeleton (`vfs`) с read-only контрактом `open/read/close/readdir`, который даёт единый API поверх storage-transport для следующего шага с минимальной файловой системой.
...
- `vfs.c/.h` — read-only VFS skeleton с базовыми контрактами `open/read/close/readdir`.
