...
- В `ui` выделен layout-контракт (`ui_layout_t`), который задаёт регионы панели, контента, статусного окна и footer-строк отдельно от процедурной отрисовки.
- Добавлен слой тем/палитр UI (`NIGHT`/`PAPER`) с переключением профиля по клику на кнопке панели и индикацией активной темы в интерфейсе.
- Добавлен примитивный window manager в content-зоне: окна имеют `z-order`, получают фокус по клику на заголовке и поддерживают drag в пределах рабочей области.
...
- Добавлена первая минимальная read-only ФС `WOFS` поверх `storage` + `vfs`: в стандартной сборке она включена (`WOFS=1`) и монтируется лениво (по первому файловому обращению), а при необходимости диагностики можно вернуться к fallback-профилю `bootsect.bin` через `WOFS=0`.
- Добавлен упрощённый ACL-like каркас в `vfs`: ноды получили owner/group/other права, а операции `open/read/readdir` теперь проходят через единый permission-check (с root bypass для текущего kernel-only режима).
...
## Вектор развития после завершения базового плана

Базовый roadmap завершён и зафиксирован как пройденный этап в `DEVELOPMENT_PLAN.md`.
Дальнейший курс WoOS: развивать собственное ядро и архитектурный контроль, но подключать проверенные внешние модули для типовых подсистем (сеть, ФС, часть драйверов) через тонкие адаптеры.

Коротко:
- архитектура, init-flow и fallback-поведение остаются под контролем WoOS;
- типовые инфраструктурные блоки предпочтительно интегрируются, а не переписываются с нуля;
- каждая интеграция проходит через stage `drivers` и фиксируется в документации/релизных заметках.
...
Для принудительного fallback-профиля без WOFS:
```bash
make clean
make os.img WOFS=0
```
...
Начиная с `1.21.0` основной цикл стал ещё отзывчивее: idle-пауза уменьшена, а runtime-оверлей обновляется по heartbeat-тикам (вместо каждого mouse-микрошагa), поэтому курсор и клики ощущаются заметно быстрее на `virtio-vga-gl`.
...
- `vfs.c/.h` — read-only VFS с монтированием минимальной ФС `WOFS` (по умолчанию), fallback-профилем `bootsect.bin` через флаг `WOFS=0` и базовым ACL-like permission scaffold.
...
- Релизный workflow (`.github/workflows/release.yml`) автоматически запускается после merge PR и публикует релиз:
