...
- В `idt` для штатного режима оставлен размаскированным только `IRQ1` (keyboard); `IRQ2` (cascade) не включается, пока slave-линии не используются, чтобы избежать спорадических IRQ на нестабильных VM.
