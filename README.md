...
нерекомендованный видеодевайс. По итогам разбора различий `1.10.1` и `1.11.0` основная
регрессия исправлена в `1.15.2`: IRQ-stub больше не портят регистры прерванного кода, а
`sti` выполняется только после регистрации обработчиков. Для WoOS сначала проверьте
базовый VGA-совместимый путь:
```bash
qemu-system-x86_64 \
  -drive format=raw,file=os.img \
  -m 1024 \
