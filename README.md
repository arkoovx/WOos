...
нерекомендованный видеодевайс. В `1.15.0` расширен перебор VBE-режимов, а в `1.15.1`
исправлена загрузка больших `kernel.bin` (чтение с диска без перехода через 64 KiB-границу
одним BIOS-вызовом), из-за чего регрессия после `1.10.1` на части QEMU-конфигураций
должна исчезнуть. Для WoOS сначала проверьте базовый VGA-совместимый путь:
```bash
qemu-system-x86_64 \
  -drive format=raw,file=os.img \
  -m 1024 \
