...
- Добавлена первая минимальная read-only ФС `WOFS` поверх `storage` + `vfs`, но в текущем стабильном профиле она отключена по умолчанию (`WOFS=0`) до завершения диагностики reboot-loop; штатный VFS использует безопасный fallback-файл `bootsect.bin`.
...
Для экспериментального включения WOFS:
```bash
make clean
make os.img WOFS=1
```
...
- `vfs.c/.h` — read-only VFS с безопасным fallback (`bootsect.bin`) и опциональным монтированием минимальной ФС `WOFS` (флаг `WOFS=1`).
