...
## 1.15.2
- Проведён точный разбор регрессии между `1.10.1` и `1.11.0`: критичным изменением оказался новый IRQ-путь (`idt_asm.asm`), где ISR-stub сохраняли только `rax` и вызывали C-код, повреждая регистры прерванного контекста.
- Исправлены IRQ-stub (`IRQ1/IRQ12`): теперь перед вызовом `idt_dispatch_irq` сохраняются и восстанавливаются все GPR, чтобы прерывания не портили состояние ядра.
- Включение прерываний вынесено из `idt_init` в отдельный `idt_enable_interrupts()` и перенесено в `kmain` после регистрации IRQ-handlers; это убирает раннее окно с неинициализированными обработчиками.
- Обновлены `VERSION` и UI-баннер до `1.15.2`.
