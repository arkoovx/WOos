...
## 1.15.2
- Исправлена регрессия IRQ-path, появившаяся после `1.10.1`: asm IRQ-stub теперь сохраняют и восстанавливают полный набор GPR перед/после вызова `idt_dispatch_irq`, чтобы асинхронные аппаратные прерывания не повреждали прерванный контекст.
- Убрано раннее «опасное окно» включённых прерываний: `sti` вынесен из `idt_init()` в `idt_enable_interrupts()`, а включение IRQ перенесено в `kmain` после `mouse_init(...)` и регистрации `idt_set_irq_handler(...)`.
- Обновлены `VERSION`, `README.md` и UI-баннер до `1.15.2`.

## 1.15.1
- Найдена и исправлена реальная причина регрессии после `1.10.1`: stage1 (`boot.asm`) читал весь `kernel.bin` одним вызовом `INT 13h AH=42h` в буфер `0000:8000`. Когда образ стал больше ~64 секторов (как в `1.11.0+`), чтение пересекало границу 64 KiB и на части BIOS/QEMU приводило к некорректной загрузке и симптому `Display output is not active`.
- Загрузчик переведён на чтение payload порциями (chunked read, до 127 секторов за вызов) с корректным обновлением destination segment и LBA между итерациями.
- Обновлены `README.md`, `VERSION` и UI-баннер до `1.15.1`.

## 1.15.0
- Расширен список VBE fallback-режимов в `stage2.asm`: помимо `0x118/0x117/0x115/0x114` теперь пробуются `0x11B/0x11A` и гарантированные низкие режимы `0x112/0x111/0x110`.
- Это устраняет сценарий запуска через `-device virtio-vga-gl`, когда BIOS/QEMU не предоставляет прежний набор VBE-режимов и QEMU пишет `Display output is not active`.
- Обновлены `VERSION` и UI-баннер до `1.15.0`.

## 1.14.0
- Исправлен stage2 VBE init: вместо жёсткой попытки только режима `0x118` добавлен перебор списка совместимых VBE-режимов (`0x118/0x117/0x115/0x114`) с переходом на первый успешный. Это устраняет ранний halt на части конфигураций QEMU, где конкретный mode отсутствует.
- В `README.md` добавлены пояснения и рабочая команда запуска для случая, когда QEMU показывает `Display output is not active`.
- Обновлены `VERSION` и UI-баннер до `1.14.0`.
